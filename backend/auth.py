...
logger.info(
    f"Using SUPABASE_KEY: {SUPABASE_KEY[:5]}...{SUPABASE_KEY[-5:] if len(SUPABASE_KEY) > 10 else ''}"
)
...
async def verify_token(token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Token payload (decoded without verification): {decoded_payload}"
            )
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"REST API authentication failed: {response.status_code} - {response.text}"
                )
        except Exception as e:
